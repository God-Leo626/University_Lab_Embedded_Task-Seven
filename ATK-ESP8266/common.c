...
const u8* wifista_ssid="A304";			//路由器SSID号
const u8* wifista_encryption="wpawpa2_aes";	//wpa/wpa2 aes加密方式
const u8* wifista_password="304666666"; 	//连接密码
...
void atk_8266_at_response(u8 mode)
{
	...
	{ 
		...
		printf("%s",USART3_RX_BUF);	//发送到串口                     //111111122222333333444444555555666666667777889999
...
u8 atk_8266_send_cmd(u8 *cmd,u8 *ack,u16 waittime)
{
	u8 res=0; 
	USART3_RX_STA=0;
	u3_printf("%s\r\n",cmd);	//发送命令                           //111111122222333333444444555555666666667777889999
...
u8 atk_8266_send_data(u8 *data,u8 *ack,u16 waittime)          
{
	u8 res=0; 
	USART3_RX_STA=0;
	u3_printf("%s",data);	//发送命令                              //111111122222333333444444555555666666667777889999
	if(ack&&waittime)		//需要等待应答
	{
		while(--waittime)	//等待倒计时
		{
